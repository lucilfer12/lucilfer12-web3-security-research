...
def main() -> int:
    ...
    if args.command == "inventory":
        ...
        _print_json(value) if args.json else print(
            f"cases={value['case_count']} reproducible={value['reproducible_cases']} "
            f"knowledge={sum(value['knowledge_registry_counts'].values())}"
        )
        return 0

    if args.command == "query":
        query = CaseQuery(args.category, args.status, args.record_type, args.tag, args.text)
        value = summarize_cases(query_cases(_root(args.path), query))
        _print_json(value) if args.json else print("\n".join(
            f"{x['id']} [{x['status']}] {x['title']}" for x in value
    ...
    if args.command == "ledger":
        ...
        event = append_event(_root(args.path), event_type=args.event_type, subject=node,
